...
"use client"
import { Gamepad2, RotateCw, Timer, Key, Copy, Link, ArrowRight } from "lucide-react";
...
export default function CreateGamePage() {
    ...
    const handleCreateRoom = async () => {
        if (!selectedRounds || !selectedTimer) {
            alert("Please select rounds and timer")
            return
        }
        ...
        setTimeout(() => {
            setRoomCode("XK9P") // This will come from API later
            setIsCreating(false)
        }, 1500)
    }
    return (
            ...
            <main className="w-full max-w-225 glass-panel rounded-3xl relative overflow-hidden flex flex-col md:flex-row shadow-2xl ring-1 ring-white/10">
                ...
                <div className="flex-1 p-6 md:p-10 flex flex-col bg-background-light/5 dark:bg-background-dark/40">
                    ...
                    <form className="flex flex-col h-full justify-between gap-8">
                                ...
                                <div className="grid grid-cols-4 gap-3">
                                    {
                                        roundOptions.map((round) => (
                                            <Button variant={round === selectedRounds ? "primary" : "outline"} key={round} onClick={() => setSelectedRounds(round)} type="button">{round}</Button>
                                        ))
                                    }
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-4 gap-3">
                                    {
                                        timerOptions.map((timer) => (
                                            <Button variant={timer === selectedTimer ? "primary" : "outline"} key={timer} onClick={() => setSelectedTimer(timer)} type="button">{timer}</Button>
                                        ))
                                    }
                                </div>
                            </div>
                        ...
                        {roomCode && <div className="space-y-5">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                            ...
                            <div className="relative group cursor-pointer w-full">
                                ...
                                <div className="relative bg-[#0a1108]/80 border border-white/10 rounded-2xl p-5 flex flex-col items-center justify-center gap-2 hover:border-primary/50  group-active:scale-[0.99] transition-transform">
                                    <div className="text-6xl font-black text-white tracking-[0.25em] pl-4 font-mono neon-text-shadow">XK9P</div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-2 bg-black/20 border border-white/10 rounded-2xl p-1.5 pr-2 focus-within:border-primary/50 focus-within:bg-black/40 focus-within:shadow-[0_0_15px_rgba(70,236,19,0.1)] transition-all">
                                    ...
                                    <input className="bg-transparent border-none text-white/80 text-sm font-medium focus:ring-0 w-full placeholder-white/20 select-all font-mono" type="text" value="icallon.com/play/XK9P" />
                                    <button className="h-10 px-5 bg-white/10 hover:bg-white/20 active:bg-white/25 text-white text-sm font-bold rounded-xl transition-all flex items-center gap-2 shadow-sm border border-white/5">
                                        Copy Link
                                    </button>
                                </div>
                            </div>
